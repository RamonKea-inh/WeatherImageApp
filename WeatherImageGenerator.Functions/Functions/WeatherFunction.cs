...
public class WeatherFunction : BaseFunction
{
    ...
    [Function("GetWeather")]
    public Task<HttpResponseData> GetWeather(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "weather")] HttpRequestData req)
    {
        return ExecuteAsync(req,
            () => _weatherFacade.GetWeatherInformationAsync(),
            "GetWeather");
    }

    [Function("GetWeatherStations")]
    public Task<HttpResponseData> GetWeatherStations(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "weather/stations")] HttpRequestData req)
    {
        return ExecuteAsync(req,
            () => _weatherFacade.GetWeatherStationsAsync(),
            "GetWeatherStations");
    }

    [Function("GetWeatherStation")]
    public Task<HttpResponseData> GetWeatherStation(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "weather/stations/{stationId}")]
        HttpRequestData req,
        string stationId)
    {
        return ExecuteAsync(req,
            () => _weatherFacade.GetStationByIdAsync(stationId),
            "GetWeatherStation");
    }
}